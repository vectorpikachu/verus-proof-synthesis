...
from utils import clean_code, code_change_is_safe, get_nonlinear_lines, fix_one_type_error_in_code, insert_loop_isolation, insert_lemma_func
import openai
    ...
    def repair_special_assertion_error(self, code: str, verus_error:VerusError, num=1, temp=1.0) -> str:
        ...
        if ".filter(" in assertion_info and ".subrange(" in code and not ".subrange(" in assertion_info:
            ...
            if not "lemma_seq_subrange_all" in code:
                newcode = insert_lemma_func(code, ["seq_subrange_all"], self.config.lemma_path)
        ...
        if ".take(" in assertion_info:
            if not "lemma_seq_take_ascend" in code and not "lemma_seq_take_all" in code:
                newcode = insert_lemma_func(code, ["seq_take_ascend", "seq_take_all"], self.config.lemma_path)
            elif not "lemma_seq_take_all" in code:
                newcode = insert_lemma_func(code, ["seq_take_all"], self.config.lemma_path)
            elif not "lemma_seq_take_ascend" in code:
                newcode = insert_lemma_func(code, ["seq_take_ascend"], self.config.lemma_path)
            else:
                newcode = code 
        ...
        if ".subrange(" in assertion_info:
            newcode = insert_lemma_func(code, ["seq_subrange_ascend", "seq_subrange_all"], self.config.lemma_path)
            newcode = self.repair_assertion_error_with_lemma_func(newcode, verus_error, 1, 
        ...
        if ".contains(" in assertion_info:
            newcode = insert_lemma_func(code, ["vec_push", "vec_remove"], self.config.lemma_path)
        ...
        if ".subrange(" in assertion_info:
            if not "lemma_seq_subrange_ascend" in code and not "lemma_seq_subrange_all" in code:
                ...
                newcode = insert_lemma_func(code, ["seq_subrange_ascend"], self.config.lemma_path)
            else:
                newcode = code
...
Response with the Rust code only, do not include any explanation."""
    ...
    def run_code(self, code, func_name=None, repair_num=5):
        ...
        code = self.repair_veval(code, max_attempt = repair_num, func_name=func_name, temp_dir=temp_dir)
